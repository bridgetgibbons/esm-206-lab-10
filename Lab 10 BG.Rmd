...
env_var <- read_csv("world_env_vars.csv") %>% 
  na.omit  #Don't ever do this for real data without looking at it first!
...
```{r donner_data}
DonnerTable <- read_csv("DonnerTable.csv") #Pay close attention to this for the final!!! Coerce binary outcome in data first into 0s and 1s - look at Donner Table outcome as an example
...
1 = Survival
0 = Death
Model probability of that outcome based on sex and age
...
#Neg - odds going down compared to reference level
#Pos - odds going up compared to reference level
#For continuous, odds change with 1 year added

#Odds of survival decreased as age increased
#Odds of male survival less than female survival
...
#Data frame must have the exact same variable names that the original model is looking for
...
```{r new_data}
#Possibly use this on the final!!!!
...
#For finalization, maybe have y axis range from 0 to 1
...
```{r date_time}
#Use lubridate with dates/times so that R understands them
